...
def score_agent(agent: AgentIdentity) -> PostureFinding:
    ...
    if not agent.managed:
        risk += 0.32
        reasons.append("shadow_agent")
    if agent.owner is None or agent.sponsor is None:
        risk += 0.27
        reasons.append("orphaned_identity")
    if agent.last_active_days > 90:
        risk += 0.14
        reasons.append("dormant_identity")
    if agent.environment == "production" and agent.autonomy_level >= 3:
        risk += 0.10
        reasons.append("high_autonomy_production")
    if agent.external_destinations:
        risk += 0.12
        reasons.append("external_egress_capability")
    if len(agent.permissions) >= 6:
        risk += 0.10
        reasons.append("broad_permission_set")
    ...
    if "data.export" in agent.permissions and agent.external_destinations:
        risk += 0.08
        reasons.append("sensitive_data_to_external_path")
...
def posture_summary(agents: list[AgentIdentity]) -> dict[str, int | float]:
    ...
    return {
        "agents": len(agents),
        "managed": sum(a.managed for a in agents),
        ...
        "mean_risk": round(sum(f.risk for f in findings) / len(findings), 4) if findings else 0.0,
    }
